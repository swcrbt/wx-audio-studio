...
| 码率 | `encodeBitRate` | 默认 48000；**与采样率强绑定**，配错直接录音失败（完整匹配关系见下表） | [S5](#17-官方来源清单) |
...
**采样率与码率的官方匹配关系**（来源 [S5](#17-官方来源清单)；本项目只用 16000/22050/44100 三档，取值靠区间上沿保留余量）：
...
| 11025 | 16000～48000 | — |
| 12000 | 24000～64000 | — |
| 16000 | 24000～96000 | 48000 |
| 22050 | 32000～128000 | 64000 |
| 24000 | 32000～128000 | — |
| 32000 | 48000～192000 | — |
| 44100 | 64000～320000 | 96000 |
| 48000 | 64000～320000 | — |
...
| `sampleRate` | 默认 8000；PC 不支持设置 | 16000 / 22050 / 44100 可选（默认 44100） | ✅ 三档均在官方合法值内；需 PC 降级路径 |
| `numberOfChannels` | 默认 2 | 1 | ✅ |
| `format` | 默认 aac | PCM | ✅ |
| `frameSize` | 单位 KB，仅 mp3/pcm 支持 | 64 | ✅ 与 PCM 匹配 |
| `encodeBitRate` | 默认 48000；与采样率强绑定 | 按采样率查表：16000→48000、22050→64000、44100→96000 | ✅ 三档均在合法区间 |
...
> 上面这四条路径的**具体操作步骤、上机顺序与逐项清单**见 [07 · M0 真机验证手册](./07-m0-verification.md)；本节只定义实验本身。
...
| DB-02 | 长音频解码的内存上限？ | 逐步解码 1/2/3/5/8 分钟 44.1k **单声道**（立体声等价于时长减半；用单声道是为了控制生成素材自身的内存峰值），记录成功/失败点与耗时 | 得出可安全解码的最大时长（换算成立体声口径） | 内存策略（第 3 节） |
