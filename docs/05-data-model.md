...
> 状态：**已定稿** ｜ 最后更新：2026-09-12 ｜ 关联代码：未实现（设计阶段，规划于 `miniprogram/core/types.ts`、`core/edl/`、`core/fs/`）
>
...
| 项 | 值 / 策略 |
| --- | --- |
| 单次写入上限 | 100MB（平台硬限制，错误码 1300202） |
...
| 原子性 | 先写 `{id}.json.tmp` 再 `rename`（若平台支持 `rename`；否则"先写新文件 → 写成功后删旧"）⚠️ 待验证 `FileSystemManager.rename` 行为 |
...
| 时间轴出现负数或重叠错误 | 所有写入 `timelineStart` 的路径统一走 `edlOps` 层并做 clamp + 排序，禁止在 UI 层直接改 EDL |
