/**
 * 峰值金字塔构建（数据结构与算法见 docs/03 §4）。
 *
 * 结构：每个桶存该区间内 PCM 的 `min`/`max`（Int16，交错排列）；
 * 每上一级把下一级的 **2 个桶合并为 1 个**，因此 `bucketSize` 逐级 ×2：
 *
 * | level | bucketSize（采样帧/桶） |
 * | --- | --- |
 * | 0 | `BASE_BUCKET`（1024） |
 * | 1 | 2048 |
 * | 2 | 4096 |
 * | … | `BASE_BUCKET << level` |
 *
 * 合并只取 min 的较小者与 max 的较大者，因此**向上合并没有任何精度损失**（相对本级而言）。
 */

/** 基础桶大小（采样帧/桶），见 docs/03 §4.1。 */
